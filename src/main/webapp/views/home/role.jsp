...
<body>
	<section class="wrapper site-min-height">
		<!-- page start-->
		<section class="panel">
			<header class="panel-heading"> Manage Role </header>
								...
								<ul>
									...
									<c:if test="${pageid != 1}">
										<li class="prev active"><a
											href="<%=request.getContextPath() + UriServlet.ROLE%>?pageid=${pageid - 1}&keyword=<%=request.getParameter("keyword")%>">←
												Prev</a></li>
									</c:if>
									<c:forEach var="i" begin="1" end="${totalPage}">
										<c:choose>
											<c:when test="${pageid == i}">
												<li class="active"><a
													href="<%=request.getContextPath() + UriServlet.ROLE%>?pageid=${i}&keyword=<%=request.getParameter("keyword")%>">${i}</a></li>
											</c:when>
											<c:otherwise>
												<li><a
													href="<%=request.getContextPath() + UriServlet.ROLE%>?pageid=${i}&keyword=<%=request.getParameter("keyword")%>">${i}</a></li>
									...
									<c:if test="${pageid != totalPage}">
										<li class="next active"><a
											href="<%=request.getContextPath() + UriServlet.ROLE%>?pageid=${pageid + 1}&keyword=<%=request.getParameter("keyword")%>">Next
												→ </a></li>
									</c:if>
								</ul>
							</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form class="form-horizontal" role="form"
						action="<%=request.getContextPath() + UriServlet.ROLE_ADD %>"
						method="post">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<form class="form-horizontal"
						action="<%=request.getContextPath() + UriServlet.ROLE_EDIT%>"
